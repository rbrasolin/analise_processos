...
function iniciarPaginaPaisagem(doc, titulo = "") {
...
function desenharTabelaPDF(doc, config) {
  ...
  const drawHeaderBlock = (yHeaderTop) => {
    if (pageHeader) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(13);
      doc.setTextColor(17, 24, 39);
      doc.text(pageHeader, margem, margem);
    }

    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.setTextColor(17, 24, 39);
    doc.text(titulo, x, yHeaderTop);

    const yHeader = yHeaderTop + gapTituloCabecalho;
  ...
  rows.forEach((row) => {
    let maxLines = 1;
    const lineCache = columns.map((col, i) => {
...
async function baixarAnalisePDF() {
  ...
  y = desenharTabelaPDF(doc, {
    ...
    margem,
    fontSize: 9,
    pageHeader: "Análise do Processo"
  });
  ...
  let landscape = iniciarPaginaPaisagem(doc, "Análise do Processo");
  ...
  landscape = iniciarPaginaPaisagem(doc, "Análise do Processo");
  ...
  yL = desenharTabelaPDF(doc, {
    ...
    columns: [
      ...
      { header: "Categoria", key: "categoriaFmt", weight: 1.9, align: "left" },
      { header: "Ganho", key: "ganhoFmt", weight: 1.0, align: "center" },
      { header: "To Be", key: "tempoToBeFmt", weight: 1.0, align: "center" },
      { header: "Observação", key: "observacao", weight: 2.8, align: "left" }
    ],
    rows: [
